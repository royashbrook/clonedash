...
test("a scaled piece grows about its own centre; absent and 1 are the same shape", () => {
  const block = object("block", 5, 2);
  assert.deepEqual(polygon({ ...block, scale: 1 }), polygon(block));
  const b = bounds({ ...block, scale: 2 });
  assert.deepEqual(b, { left: 4.5, right: 6.5, bottom: 1.5, top: 3.5 });
  const spike = bounds({ ...object("spike", 5, 2), scale: 0.5 });
  assert.deepEqual(spike, { left: 5.25, right: 5.75, bottom: 2.25, top: 2.75 });
  const ramp = bounds({ ...object("ramp", 5, 2), scale: 3 });
  assert.deepEqual(ramp, { left: 4, right: 7, bottom: 1, top: 4 });
...
test("a big scaled block supports the player past the old 2-cell broad-phase window", () => {
  // a 4x block anchored at x=8 spans 6.5..10.5 and its top is at 2.5. standing near its right end
  // puts the player more than 2 cells from the anchor, which the old cull skipped: the player
  // would fall through the block's trailing edge.
  const slab = { ...object("block", 8, 0), scale: 4 };
  const s = { ...createState(level([slab])), x: 10.2, y: 2.5, vy: 0, grounded: true };
  for (let i = 0; i < 4; i++) step(s, false);
  assert.ok(s.x < 10.5 && s.x - 8 > 2, "left edge still over the slab, more than 2 cells from the anchor");
  assert.equal(s.y, 2.5, "supported by the scaled block's real top");
  assert.equal(s.grounded, true);
  assert.equal(s.status, "playing");
  // the same anchor at scale 1 spans 8..9: at x=10.2 there is nothing under the player
  const s1 = { ...createState(level([object("block", 8, 0)])), x: 10.2, y: 2.5, vy: 0, grounded: true };
  for (let i = 0; i < 6; i++) step(s1, false);
  assert.ok(s1.y < 2.5, "falls, so the support above really came from the scale");
});
