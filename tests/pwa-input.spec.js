...
test('play surface blocks selection and the zoom gesture; text inputs stay editable', async ({ page }) => {
  await page.goto('/');

  const body = await page.evaluate(() => {
    const s = getComputedStyle(document.body);
    return { userSelect: s.userSelect || s.webkitUserSelect, callout: s.getPropertyValue('-webkit-touch-callout') };
  });
